...
-- 1. 用户每日汇总表
CREATE TABLE IF NOT EXISTS dws_user_day_agg (
    user_id         BIGINT      COMMENT '用户ID',
    user_nick_name  STRING      COMMENT '用户昵称',
    user_level      STRING      COMMENT '用户等级',
    gender          INT         COMMENT '性别',
    order_count     INT         COMMENT '当日下单数',
    order_amount    DECIMAL(10,2) COMMENT '当日下单金额',
    pay_amount      DECIMAL(10,2) COMMENT '当日支付金额',
    item_count      INT         COMMENT '当日购买商品数',
...
-- 2. 商品每日汇总表
CREATE TABLE IF NOT EXISTS dws_sku_day_agg (
...
-- 3. 订单每日汇总表
CREATE TABLE IF NOT EXISTS dws_order_day_agg (
...
-- 4. 分类每日汇总表
CREATE TABLE IF NOT EXISTS dws_category_day_agg (
...
-- 2. 用户看板
CREATE TABLE IF NOT EXISTS ads_user_dashboard (
    dt                  STRING  COMMENT '日期',
...
-- 3. 商品销售排行榜
CREATE TABLE IF NOT EXISTS ads_sku_sales_top (
    dt              STRING      COMMENT '日期',
